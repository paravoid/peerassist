...
def peering_matrix(us, them):
    table = PrettyTable(["IXP", "ASN L", us.name, "ASN R", them.name, "Established",])
...
def email(us, them):
    ...
    existing_ixp = common_ixp.filter(ixlan_set__netixlan_set__peering__in=peerings)
    new_ixp = common_ixp.exclude(ixlan_set__netixlan_set__peering__in=peerings)
